...
def generate_random_dataset(rng: jax.random.PRNGKey, num_samples: int) -> DiffusionDataset:
...
def save_toy_dataset(num_samples: int = 1000):
    ...
    with h5py.File("toy_dataset.h5", "w") as f:
        ...
        f.create_dataset('U', (0, 49, 2), maxshape=(None, 49, 2), dtype='float32')
        f.create_dataset('s', (0, 49, 2), maxshape=(None, 49, 2), dtype='float32')
        f.create_dataset('sigma', (0, 1), maxshape=(None, 1), dtype='float32')
        f.create_dataset('k', (0, 1), maxshape=(None, 1), dtype='float32')

    jit_generate = jax.jit(lambda rng: generate_random_dataset(rng, num_samples))
